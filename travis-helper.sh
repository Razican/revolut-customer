...
# Upload code coverage report for stable builds in Linux.
  ...
  if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Test" &&
    ...
    cd ../.. &&
    rm -rf kcov-master &&
    for file in target/debug/amount-*[^\.d] target/debug/private-*[^\.d] target/debug/public-*[^\.d] target/debug/revolut_customer-*[^\.d]; do
...
# Upload development documentation for the develop branch.
  ...
  echo "<meta http-equiv=refresh content=0;url=revolut_customer/index.html>" > target/doc/index.html
